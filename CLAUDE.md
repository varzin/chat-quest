...
- Использует глобальные настройки typing delays из storage
- Перезапуск всегда разрешен (allowRestart всегда true)
...
- Детекция структурных паттернов в ответах персонажа (_detectPattern — эвристика без API)
...
- Глобальные настройки (theme, language, typingMinDelay, typingMaxDelay)
- Значения по умолчанию: typingMinDelay=400ms, typingMaxDelay=1600ms
...
4. **Settings Modal** - настройки (тема, скорость печати, язык, очистка данных)
...
- Фреймворк: `node:test` + `node:assert` (встроены в Node 18+, ноль зависимостей)
- Файлы тестов: `tests/*.test.mjs`
- Запуск всех тестов: `node --test tests/*.test.mjs`
- Запуск одного файла: `node tests/ai-engine.test.mjs`
